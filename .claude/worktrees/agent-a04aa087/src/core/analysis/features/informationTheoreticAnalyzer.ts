...
// Lazy-load zlib to avoid crashing in browser (Vite externalizes Node built-ins)
type DeflateSyncFn = (input: Uint8Array, options?: { level: number }) => Buffer;
let _deflateSync: DeflateSyncFn | null | undefined; // undefined = not yet checked

function getDeflateSync(): DeflateSyncFn | null {
  if (_deflateSync !== undefined) return _deflateSync;
  try {
    // Use indirect require to hide from Vite's static analysis
    const req = typeof require === 'function' ? require : null;
    _deflateSync = req ? (req('zlib') as { deflateSync: DeflateSyncFn }).deflateSync : null;
  } catch {
    _deflateSync = null;
  }
  return _deflateSync;
...
function compressedSize(text: string): number {
  ...
  const fn = getDeflateSync();
  if (fn) {
    try {
      const compressed = fn(Buffer.from(text, 'utf-8'), { level: 6 });
      return compressed.length;
    } catch {
      // Fall through to estimation
    }
  }
  // Browser fallback: entropy-based compression estimate
  ...
  // Theoretical minimum bits per byte * length, with ~10% deflate overhead
  return Math.max(1, Math.ceil((entropy / 8) * bytes.length * 1.1));
}
...
function computeRubricScores(
  ...
  // structural_cohesion: NCD balance + density variation
  // TODO: MI formula uses sum of marginals, not joint probability — MI coherenceScore
  // excluded from rubric scoring until the formula is corrected. Weights redistributed
  // to NCD (0.55) and density variation (0.45).
  const structuralBalance = round(
    ncd.balanceScore * 0.55 + densityVariation.variationScore * 0.45,
    1
  );
  ...
  // TODO: MI introConclusion uses flawed joint probability formula — using NCD-based
  // proxy instead. Adjacent-paragraph NCD gives a reasonable structural coherence signal.
  const introConclCoherence = round(clamp(ncd.balanceScore * 0.8 + compression.uniquenessScore * 0.2, 0, 10), 1);
