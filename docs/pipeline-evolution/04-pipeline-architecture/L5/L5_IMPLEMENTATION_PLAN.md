...
- **Contract:** Handle the new source `'analysis_specifics_gap'` and the three new statuses. Add a `getOpenAnalysisGapQuestions(): UnderstandingQuestion[]` accessor. Add status-transition methods: `markAskedToStudent(id, conversatorMessageId)`, `markStudentAnswered(id, rawAnswer, structuredAnswer)`, `markStudentDeclined(id, reason)`. Each transition is validated (illegal transitions throw — e.g., can't go from `'resolved'` to `'asked_to_student'`). *[Spec amendment 2026-05-01: `markStudentAnswered` widened from `(id, structuredAnswer)` to `(id, rawAnswer, structuredAnswer)`. Reason: `DigContext.studentAnswerRaw` (`profileTypes.ts:5759`) is documented as "persisted before extraction in case extraction fails," and `DigContext.extractionPending` (`:5772`) requires `rawAnswer` for retry-without-re-asking. The spec's literal `(id, structuredAnswer)` would have forced callers to either drop the raw text on success (losing retry capability) or call a separate setter. The widening preserves the type's full persistence contract atomically. Documented inline at `questionQueueManager.ts` D-2.1 closure section.]*
...
- **Blocks:** D-2.8.
- **Contract:** Pure deterministic. Inputs: per-layer specifics-need emissions. Outputs: deduplicated UnderstandingQuestion[] entries with source `'analysis_specifics_gap'`. Dedup logic: by `(anchorParagraph, expectedAnswerShape, framingSeed-similarity)`. New entries get fresh IDs; existing open queue questions matching new emissions get `iterationsSurvived++`. Schema-validate every emission; malformed → throw with layer + index context. *[Spec amendment 2026-05-01: framingSeed-similarity := Jaccard similarity over Unicode-aware lowercased word tokens of length ≥ 3, threshold ≥ 0.5. The threshold is conservative because failure modes are asymmetric — a false-merge silently erases a distinct gap (audit-only signal loss); a false-split adds a redundant question that mergeCuratedOutput's auto-promotion (iterationsSurvived ≥ 3 → priority='high') self-heals on the next iteration. Tunable via the `FRAMING_SEED_SIMILARITY_THRESHOLD` constant in the module; should NOT vary per-layer or per-essay (would re-introduce a closed-taxonomy antipattern). Also: a new `DigContext.anchorSentence?: number` field landed in the same closure (round-1 audit HIGH-1) so emissions carrying sentence-level anchor granularity persist that signal end-to-end rather than being silently dropped at mint time.]*
