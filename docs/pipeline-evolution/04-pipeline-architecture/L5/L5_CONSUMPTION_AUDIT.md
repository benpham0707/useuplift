...
| T2 → cross-iteration synthesis | Iteration response surface lede on iteration ≥ 3; uses `TaughtMove.deepenedBy[]` + `supersededBy` chain (per `L5_ITERATION_LOOP_DESIGN.md` §9.2 priority 3 promotion) | §5.9 |
...
| 253 | IterationLedger | `taughtMoves: TaughtMove[]` | same | [D] | n/a (new) | priorAnnotations builder, landing detector, cross-iteration synthesis | `bookkeeping` (append-only) | keep | The non-repetition spine. |
...
| 256 | TaughtMove | `id`, `annotationId`, `findingId?`, `location`, `taughtAtIteration`, `teachingMode`, `contentSummary`, `stakesSnapshot?`, `landing`, `deepenedBy[]`, `supersededBy?` | same | [D] | n/a (new) | priorAnnotations builder reads `landing`; cross-iteration synthesis reads `deepenedBy[]` + `supersededBy`; Conversator continuous-chat handler reads when student asks "have we worked on this before?" | `bookkeeping` (append-only; landing populated next iteration) | keep | The cross-iteration narrative + landing carrier. |
