...
| 13 | **L5 deepAnnotationService** | `analysis/deepAnnotationService.ts` (~3,900 lines); wired in orchestrator. `[2026-05-10]` priorAnnotations WIRE IS LIVE at `analysisOrchestrator.ts:1299-1300` (composer `buildPriorAnnotationsForOrchestrator` reads from iterationLedger `taughtMoves[]`); the original ":850 dead-wire" claim is stale. `[2026-05-12]` Density target locked at **20-30 L5Annotation per essay** with ~250-375 tokens per annotation (depth > breadth). | **partial** | Of 16 capabilities flagged in L5_FEEDBACK_REDESIGN.md §1.8, priorAnnotations integration is now functional. **Top-N ranking capability (item 4) PROMOTED to load-bearing for Phase 5** — required for L5 to honestly hit the 20-30 density target by selecting from a larger candidate set. ENABLE_FOCUS_MODE flag-gated reranking exists at `:795` but never set in production (Phase 8 first activation). |
