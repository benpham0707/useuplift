...
create index college_program_areas_filter_idx on public.college_program_areas (cip_area_code, institution_id);
...
create index college_profiles_program_area_codes_idx on public.college_profiles using gin (program_area_codes);
create index college_profiles_version_net_price_idx on public.college_profiles (projection_version_id, net_price, slug) where net_price is not null;
create index college_profiles_version_enrollment_idx on public.college_profiles (projection_version_id, undergraduate_enrollment, slug) where undergraduate_enrollment is not null;
...
as $function$
  select coalesce(array_agg(program.cip_area_code order by program.cip_area_label), '{}'::text[]),
    coalesce(array_agg(program.cip_area_label order by program.cip_area_label), '{}'::text[])
  from (
    select distinct on (area.cip_area_code) area.cip_area_code, area.cip_area_label
...
as $function$
begin
  select arrays.codes, arrays.labels into new.program_area_codes, new.program_area_labels
  from public.college_program_arrays(new.institution_id) arrays;
  return new;
end
$function$;
create trigger set_college_profile_program_arrays before insert or update of institution_id
  on public.college_profiles for each row execute function public.set_college_profile_program_arrays();
...
create table public.user_college_list_items (
  ...
  constraint user_college_list_items_category_check check (category is null or category in ('reach', 'match', 'safety')),
  constraint user_college_list_items_status_check check (status in ('interested', 'researching', 'applying', 'applied', 'accepted', 'denied', 'waitlisted', 'enrolled')),
  constraint user_college_list_items_position_check check (position is null or position >= 0)
);
create index user_college_list_items_user_added_idx on public.user_college_list_items (user_id, added_at desc);
...
$function$;
create trigger touch_user_college_list_item before update on public.user_college_list_items
...
revoke all on public.college_program_areas, public.user_college_list_items from public, anon, authenticated;
grant select, insert, update, delete on public.college_program_areas, public.user_college_list_items to service_role;
grant execute on function public.college_program_arrays(uuid), public.refresh_college_program_areas(uuid) to service_role;
