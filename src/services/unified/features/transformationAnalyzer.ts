...
export interface TransformationAnalysis {
  // Overall assessment
  transformation_score: number; // 0-10
 ...
 */
export function analyzeTransformation(essayText: string): TransformationAnalysis {
  ...
  // Belief shifts (+3 max)
  if (beliefShifts.count >= 2 && beliefShifts.specificity === 'concrete') score += 3.0;
  else if (beliefShifts.count >= 1 && beliefShifts.specificity === 'concrete') score += 2.0;
  else if (beliefShifts.count >= 1) score += 1.0;

  // Behavioral changes (+2 max)
  if (behaviorChanges.count >= 2) score += 2.0;
  else if (behaviorChanges.count >= 1) score += 1.0;

  // Growth arc (+2 max)
  if (growthArc.type === 'earned_through_struggle') score += 2.0;
  else if (growthArc.type === 'gradual_realization') score += 1.5;
  else if (growthArc.type === 'instant_epiphany') score += 0.5;

  // Impact (+1 max)
  const impactCount = [impact.relationships, impact.worldview, impact.identity].filter(Boolean).length;
  if (impactCount >= 2) score += 1.0;
  else if (impactCount >= 1) score += 0.5;

  // Before/after (+1 max)
  if (beforeAfter.specificity === 'concrete_with_evidence') score += 1.0;
  else if (beforeAfter.specificity === 'stated_but_vague') score += 0.5;

  // Penalties for generic learning (-3 max)
  if (genericLearning.count >= 5) score -= 3.0;
  else if (genericLearning.count >= 3) score -= 2.0;
  else if (genericLearning.count >= 1) score -= 1.0;

  // Cap at 0-10
  score = Math.max(0, Math.min(10, score));

  // Determine transformation quality
  let quality: 'world_class' | 'strong' | 'adequate' | 'stated' | 'absent';
  if (score >= 9) quality = 'world_class';
  else if (score >= 7) quality = 'strong';
  else if (score >= 5) quality = 'adequate';
  else if (score >= 3) quality = 'stated';
  else quality = 'absent';
...
function detectBeliefShifts(text: string): {
  count: number;
  examples: string[];
  ...
  let specificity: 'concrete' | 'vague' | 'absent';
  if (count === 0) specificity = 'absent';
...
function detectBehavioralChanges(text: string): {
  count: number;
  examples: string[];
...
function detectGrowthArc(text: string): {
  present: boolean;
  type: 'earned_through_struggle' | 'gradual_realization' | 'instant_epiphany' | 'stated_only' | 'none';
  examples: string[];
} {
  ...
  const examples: string[] = [];
  let type: 'earned_through_struggle' | 'gradual_realization' | 'instant_epiphany' | 'stated_only' | 'none';
...
function detectImpact(text: string): {
  ...
  if (relationshipMatches) examples.push(...relationshipMatches.slice(0, 1));
  if (worldviewMatches) examples.push(...worldviewMatches.slice(0, 1));
  if (identityMatches) examples.push(...identityMatches.slice(0, 1));
...
function detectGenericLearning(text: string): {
  count: number;
  examples: string[];
} {
  const patterns = [
...
function detectBeforeAfterComparison(text: string): {
  present: boolean;
  specificity: 'concrete_with_evidence' | 'stated_but_vague' | 'absent';
} {
