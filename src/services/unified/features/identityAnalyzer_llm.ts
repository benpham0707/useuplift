...
export interface IdentityAnalysis {
  ...
  reasoning: {
    core_values: string[]; // Values identified (e.g., "Empathy", "Rigor")
    identity_complexity: string; // Nuance analysis
    show_vs_tell_identity: string; // Demonstrated vs Stated
    authenticity_check: string; // Does it feel real?
  };
  ...
  traits: {
    ...
    contradictions: string[]; // Interesting complexities
  };
...
export async function analyzeIdentity(essayText: string): Promise<IdentityAnalysis> {
...
Provide your analysis as JSON following the exact format specified.`;
  ...
  try {
    const response = await callClaude<IdentityAnalysis>(userPrompt, {
