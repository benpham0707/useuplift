...
export interface ContextCircumstancesAnalysis {
  ...
  // Obstacle specificity
  has_specific_obstacles: boolean;
  obstacle_types: string[]; // ['financial', 'access', 'family', 'language', 'discrimination', 'disability']
 ...
 */
export function analyzeContextCircumstances(essayText: string): ContextCircumstancesAnalysis {
  ...
  if (obstacles.count >= 3) score += 2.5;
  else if (obstacles.count >= 2) score += 2.0;
  else if (obstacles.count >= 1) score += 1.0;
  else score -= 3.0; // Increased penalty: No specific obstacles mentioned in a context essay is a major weakness

  // Resourcefulness (+2.5 max)
  if (resourcefulness.count >= 3) score += 2.5;
  else if (resourcefulness.count >= 2) score += 2.0;
  else if (resourcefulness.count >= 1) score += 1.0;

  // Resilience (+2.0 max)
  if (resilience.count >= 2) score += 2.0;
  else if (resilience.count >= 1) score += 1.0;

  // Causal connections (+1.0 max)
  if (causalConnections.count >= 2) score += 1.0;
  else if (causalConnections.count >= 1) score += 0.5;

  // Victimhood tone penalty (-3.0 max)
  if (victimhood.count >= 3) score -= 3.0;
  else if (victimhood.count >= 2) score -= 2.0;
  else if (victimhood.count >= 1) score -= 1.0;

  // Vague hardship penalty (-2.0 max)
  if (vagueHardship.count >= 3) score -= 2.0;
  else if (vagueHardship.count >= 2) score -= 1.5;
  else if (vagueHardship.count >= 1) score -= 0.5;

  // Cap at 0-10
  score = Math.max(0, Math.min(10, score));

  // Determine context quality
  let quality: 'resourceful_resilience' | 'strong_navigation' | 'mixed_context' | 'vague_hardship' | 'victimhood_tone';
  if (score >= 9) quality = 'resourceful_resilience';
  else if (score >= 7) quality = 'strong_navigation';
  else if (score >= 5) quality = 'mixed_context';
  else if (score >= 3) quality = 'vague_hardship';
  else quality = 'victimhood_tone';
...
function detectSpecificObstacles(text: string): {
  present: boolean;
  count: number;
  types: string[];
  examples: string[];
  ...
  for (const pattern of financialPatterns) {
    ...
    if (matches) {
      ...
      if (!types.includes('financial')) types.push('financial');
    }
  }
  ...
  for (const pattern of accessPatterns) {
    ...
    if (matches) {
      count += matches.length;
      examples.push(...matches.slice(0, 2));
      if (!types.includes('access')) types.push('access');
    }
  }
  ...
  for (const pattern of familyPatterns) {
    ...
    if (matches) {
      count += matches.length;
      examples.push(...matches.slice(0, 2));
      if (!types.includes('family')) types.push('family');
    }
  }
  ...
  for (const pattern of languagePatterns) {
    ...
    if (matches) {
      count += matches.length;
      examples.push(...matches.slice(0, 2));
      if (!types.includes('language')) types.push('language');
    }
  }
  ...
  for (const pattern of discriminationPatterns) {
    ...
    if (matches) {
      count += matches.length;
      examples.push(...matches.slice(0, 2));
      if (!types.includes('discrimination')) types.push('discrimination');
    }
  }
  ...
  for (const pattern of disabilityPatterns) {
    ...
    if (matches) {
      count += matches.length;
      examples.push(...matches.slice(0, 2));
      if (!types.includes('disability')) types.push('disability');
    }
  }
...
function detectResourcefulness(text: string): {
  present: boolean;
  count: number;
  examples: string[];
} {
...
function detectResilience(text: string): {
  present: boolean;
  count: number;
  examples: string[];
} {
...
function detectCausalConnections(text: string): {
  present: boolean;
  count: number;
  examples: string[];
} {
...
function detectVictimhoodTone(text: string): {
  present: boolean;
  count: number;
  examples: string[];
} {
...
function detectVagueHardship(text: string): {
  count: number;
  examples: string[];
} {
  const patterns = [
