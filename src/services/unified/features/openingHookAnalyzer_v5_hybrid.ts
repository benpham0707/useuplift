...
export interface HookAnalysisResult {
  // CLASSIFICATION
  hook_type: HookType;
  hook_type_confidence: number; // 0-1
  detection_method: 'deterministic' | 'llm' | 'hybrid';

  // LLM DEEP ANALYSIS
  why_this_type: string; // LLM explains WHY it classified this way
  effectiveness_score: number; // 0-10
  hook_tier: 'weak' | 'adequate' | 'strong' | 'exceptional';

  // CRAFT ANALYSIS (LLM-generated)
  what_makes_it_work: string[]; // Specific techniques used
  what_could_be_stronger: string[]; // Growth opportunities

  // STUDENT-FACING INSIGHTS
  what_i_notice: string; // Caring observation about THEIR hook
  what_works: string; // Authentic praise
  opportunity: string; // Growth guidance
  next_step: string; // Concrete suggestion
  ...
  // MATCHED PATTERNS (if deterministic was used)
...
export async function analyzeOpeningHookV5(
  fullEssay: string,
  options: {
    forceLLM?: boolean; // Skip deterministic, go straight to LLM
  ...
  let deterministicResult: { type: HookType; confidence: number } | null = null;
  ...
  const llmAnalysis = await analyzWithLLM(
    opening,
    fullEssay,
    deterministicResult, // Pass hint to LLM
    options.essayType || 'leadership',
...
function quickDeterministicCheck(opening: string): { type: HookType; confidence: number } | null {
...
async function analyzWithLLM(
  opening: string,
  fullEssay: string,
  deterministicHint: { type: HookType; confidence: number } | null,
  essayType: string,
  depth: string
): Promise<Omit<HookAnalysisResult, 'hook_type' | 'hook_type_confidence' | 'detection_method'>> {
