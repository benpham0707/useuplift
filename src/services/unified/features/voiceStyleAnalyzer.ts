...
export interface VoiceStyleAnalysis {
  ...
  active_ratio: number; // 0-1 (1 = all active)
  active_examples: string[];
  passive_examples: string[];

  // Fancy word penalties
  fancy_word_count: number;
  fancy_word_examples: string[];

  // Sentence rhythm
  has_rhythm_variety: boolean;
  short_sentence_count: number; // 3-5 words
  long_sentence_count: number; // 20+ words
  rhythm_score: number; // 0-1 (variety indicator)

  // Conversational markers (bonus)
 ...
 */
export function analyzeVoiceStyle(essayText: string): VoiceStyleAnalysis {
  ...
  if (essaySpeak.count >= 5) score -= 3.0;
  else if (essaySpeak.count >= 3) score -= 2.0;
  else if (essaySpeak.count >= 1) score -= 1.0;

  // AI phrases penalty (-2.5 max)
  if (aiPhrases.count >= 4) score -= 2.5;
  else if (aiPhrases.count >= 2) score -= 1.5;
  else if (aiPhrases.count >= 1) score -= 0.5;

  // Active voice ratio (+3.0 max)
  if (voiceRatio.ratio >= 0.80) score += 3.0;
  else if (voiceRatio.ratio >= 0.70) score += 2.0;
  else if (voiceRatio.ratio >= 0.60) score += 1.0;
  else if (voiceRatio.ratio < 0.60) score -= 2.0; // Penalty if < 60% (was < 50%)

  // Fancy words penalty (-1.5 max)
  if (fancyWords.count >= 5) score -= 1.5;
  else if (fancyWords.count >= 3) score -= 1.0;
  else if (fancyWords.count >= 1) score -= 0.5;

  // Sentence rhythm variety (+1.5 max)
  if (rhythm.hasVariety) score += 1.5; // Only reward if BOTH short and long are present
  else if (rhythm.score >= 0.5) score -= 0.5; // Penalty for monotony (only one type dominant)

  // Conversational markers (bonus +1.0 max)
  if (conversational.count >= 3) score += 1.0;
  else if (conversational.count >= 2) score += 0.5;

  // Cap at 0-10
  score = Math.max(0, Math.min(10, score));

  // Determine voice quality
  let quality: 'authentic_distinctive' | 'strong_voice' | 'mixed_voice' | 'generic_essay' | 'ai_generated';
  if (score >= 9) quality = 'authentic_distinctive';
  else if (score >= 7) quality = 'strong_voice';
  else if (score >= 5) quality = 'mixed_voice';
  else if (score >= 3) quality = 'generic_essay';
  else quality = 'ai_generated';
...
function detectEssaySpeak(text: string): {
  present: boolean;
  count: number;
  examples: string[];
} {
...
function detectActivePassiveVoice(text: string): {
  activeCount: number;
  passiveCount: number;
  ratio: number;
...
function detectFancyWords(text: string): {
  count: number;
  examples: string[];
} {
  const patterns = [
...
function analyzeSentenceRhythm(text: string): {
  hasVariety: boolean;
  shortCount: number;
  longCount: number;
  score: number;
  ...
  let score = 0;
  if (shortCount >= 1) score += 0.5;
  if (longCount >= 1) score += 0.5;

  // Bonus for multiple of each
  if (shortCount >= 2 && longCount >= 2) score = 1.0;
...
function detectConversationalMarkers(text: string): {
  count: number;
  examples: string[];
} {
  const patterns = [
