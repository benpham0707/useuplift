...
export interface QuotableReflectionAnalysis {
  ...
  is_quotable: boolean; // Could be TED talk title

  // Formula elements
  has_transformation: boolean;      // "I used to think" → "Now I see"
  has_universal_scope: boolean;     // Beyond self (people, life, world)
  has_surprise: boolean;            // Paradox, counterintuitive truth
  has_philosophical_depth: boolean; // Abstract concepts
  has_poetic_language: boolean;     // Metaphor, parallel structure, rhythm
  has_future_application: boolean;  // Forward-looking

  // Quality indicators
  micro_to_macro: boolean;          // Specific → universal pattern (63% of admits)
  challenges_assumption: boolean;   // "I used to think X, but..."
  avoids_cliches: boolean;         // No "passion", "journey", "taught me a lot"
  ...
  // Research alignment
  harvard_maturity: boolean;        // Unusual maturity in self-reflection
  stanford_depth: boolean;          // One profound insight
  berkeley_memorable: boolean;      // AO would write it down
...
export function analyzeQuotableReflection(text: string): QuotableReflectionAnalysis {
  ...
  const level = calculateReflectionLevel(
    hasTransformation,
    hasUniversalScope,
    hasSurprise,
  ...
  return {
    ...
    weaknesses,
    current_level: level,
    next_level: Math.min(5, level + 1) as ReflectionLevel,
...
function detectTransformations(text: string): string[] {
  const found: string[] = [];
...
function detectUniversalLanguage(text: string): string[] {
  const found: string[] = [];
...
function detectSurpriseElements(text: string): string[] {
  const found: string[] = [];
  const lowerText = text.toLowerCase();
...
function detectPhilosophicalConcepts(text: string): string[] {
  const found: string[] = [];
  const lowerText = text.toLowerCase();
...
function detectMetaphors(text: string): string[] {
  const found: string[] = [];
...
function detectParallelStructures(text: string): string[] {
  const found: string[] = [];
...
function detectFutureApplication(text: string): boolean {
...
function calculateReflectionLevel(
  hasTransformation: boolean,
  hasUniversalScope: boolean,
  hasSurprise: boolean,
  hasPhilosophicalDepth: boolean,
  hasPoeticLanguage: boolean,
  avoidsCliches: boolean
): ReflectionLevel {
...
function calculateReflectionScore(
  ...
  // Bonuses
  if (transformationCount >= 2) score += 0.5;
  if (philosophicalCount >= 3) score += 0.5;
  if (hasPoeticLanguage) score += 0.5;

  return Math.min(10, score);
}

function getReflectionTier(level: ReflectionLevel): 'quotable' | 'profound' | 'thoughtful' | 'specific' | 'generic' {
  const tiers = {
    5: 'quotable' as const,
    4: 'profound' as const,
    3: 'thoughtful' as const,
    2: 'specific' as const,
    1: 'generic' as const
  };
  return tiers[level];
...
function findMostQuotableLine(
  text: string,
  parallelStructures: string[],
  metaphors: string[]
): string | null {
  ...
  for (const sentence of sentences) {
    const metaphorCount = metaphors.filter(m =>
...
function extractReflectionQuotes(
  text: string,
  ...
  for (const sentence of sentences) {
    const hasTransformation = transformationPhrases.some(t =>
      sentence.toLowerCase().includes(t.toLowerCase().replace(/"/g, ''))
    );
    const hasUniversal = universalLanguage.some(u =>
      sentence.toLowerCase().includes(u.toLowerCase().replace(/"/g, ''))
    );
...
function identifyReflectionStrengths(
  level: ReflectionLevel,
  hasTransformation: boolean,
  microToMacro: boolean,
  hasPoeticLanguage: boolean,
  avoidsCliches: boolean
): string[] {
...
function identifyReflectionWeaknesses(
  level: ReflectionLevel,
  hasUniversalScope: boolean,
  hasSurprise: boolean,
  avoidsCliches: boolean,
...
function generateUpgradeToQuotable(
  currentLevel: ReflectionLevel,
  hasTransformation: boolean,
  hasSurprise: boolean,
  hasPoeticLanguage: boolean
): QuotableReflectionAnalysis['upgrade_to_quotable'] {
...
// Already exported inline above (line 182)
