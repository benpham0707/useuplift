...
export interface SpecificityAnalysis {
  ...
  reasoning: {
    quantitative_analysis: string; // Numbers, metrics, impact data
    qualitative_analysis: string; // Names, places, sensory details
    showing_vs_telling_analysis: string; // Scene vs Summary
    precision_analysis: string; // Word choice (e.g., "soldered" vs "made")
  };
  ...
  // Evidence
  evidence_quotes: string[]; // Best examples of specificity
  evaluator_note: string;

  // Detailed analysis
  details: {
    metrics_count: number; // Estimate
    proper_nouns_count: number; // Estimate
    sensory_details_count: number; // Estimate
  };
...
export async function analyzeSpecificity(essayText: string): Promise<SpecificityAnalysis> {
...
Provide your analysis as JSON following the exact format specified.`;
  ...
  try {
    const response = await callClaude<SpecificityAnalysis>(userPrompt, {
