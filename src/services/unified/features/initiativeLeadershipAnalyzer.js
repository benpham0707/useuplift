 ...
 */
export function analyzeInitiativeLeadership(essayText) {
    ...
    if (problemIdentification.count >= 2)
        score += 2.0;
    else if (problemIdentification.count >= 1)
        score += 1.0;
    // Opportunity creation (+2.5 max)
    if (opportunityCreation.count >= 2)
        score += 2.5;
    else if (opportunityCreation.count >= 1)
        score += 1.5;
    // Self-directed action (+2.0 max)
    if (selfDirection.count >= 3)
        score += 2.0;
    else if (selfDirection.count >= 2)
        score += 1.5;
    else if (selfDirection.count >= 1)
        score += 0.5;
    // Risk-taking (+1.5 max)
    if (riskTaking.count >= 2)
        score += 1.5;
    else if (riskTaking.count >= 1)
        score += 1.0;
    // Informal leadership (+1.5 max)
    if (informalLeadership.count >= 2)
        score += 1.5;
    else if (informalLeadership.count >= 1)
        score += 0.5;
    // Reactive language penalty (-3.0 max)
    if (reactiveLanguage.count >= 4)
        score -= 3.0;
    else if (reactiveLanguage.count >= 2)
        score -= 2.0;
    else if (reactiveLanguage.count >= 1)
        score -= 1.0;
    // Title without initiative penalty (-1.5)
    if (titleMentions.present &&
        !opportunityCreation.present &&
        selfDirection.count === 0) {
        score -= 1.5; // Has title but no proactive action
    }
    // Cap at 0-10
    ...
    let quality;
    if (score >= 9)
        quality = 'proactive_builder';
    else if (score >= 7)
        quality = 'strong_initiative';
    else if (score >= 5)
        quality = 'mixed_leadership';
    else if (score >= 3)
        quality = 'reactive_participation';
    else
        quality = 'passive_member';
    // Generate guidance
    const strengths = [];
    const weaknesses = [];
    const quickWins = [];
...
// HELPER FUNCTIONS
// ============================================================================
function detectProblemIdentification(text) {
    const patterns = [
...
function detectOpportunityCreation(text) {
    const patterns = [
...
function detectSelfDirection(text) {
    const patterns = [
...
function detectRiskTaking(text) {
    const patterns = [
...
function detectInformalLeadership(text) {
    const patterns = [
...
function detectReactiveLanguage(text) {
    const patterns = [
...
function detectTitleMentions(text) {
    const patterns = [
...
//# sourceMappingURL=initiativeLeadershipAnalyzer.js.map