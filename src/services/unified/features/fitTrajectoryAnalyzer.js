 ...
 */
export function analyzeFitTrajectory(essayText) {
    ...
    if (futureConnection.count >= 2)
        score += 2.5;
    else if (futureConnection.count >= 1)
        score += 1.5;
    else
        score -= 2.0; // No future connection mentioned
    // Trajectory clarity (+2.5 max)
    if (trajectory.strength === 'explicit')
        score += 2.5;
    else if (trajectory.strength === 'implied')
        score += 1.5;
    else if (trajectory.strength === 'weak')
        score += 0.5;
    else
        score -= 1.5; // No trajectory
    // Continued commitment (+2.0 max)
    if (commitment.count >= 2)
        score += 2.0;
    else if (commitment.count >= 1)
        score += 1.0;
    // UC mentions (bonus +1.0 max)
    if (ucMentions.count >= 2)
        score += 1.0;
    else if (ucMentions.count >= 1)
        score += 0.5;
    ...
    if (genericMission.count >= 3)
        score -= 2.5;
    else if (genericMission.count >= 2)
        score -= 1.5;
    else if (genericMission.count >= 1)
        score -= 0.5;
    // Cap at 0-10
    ...
    let quality;
    if (score >= 9)
        quality = 'clear_trajectory';
    else if (score >= 7)
        quality = 'strong_connection';
    else if (score >= 5)
        quality = 'some_connection';
    else if (score >= 3)
        quality = 'generic_mission';
    else
        quality = 'no_future_link';
    // Generate guidance
    const strengths = [];
    const weaknesses = [];
    const quickWins = [];
...
// HELPER FUNCTIONS
// ============================================================================
function detectFutureConnection(text) {
    const examples = [];
    const types = [];
    let count = 0;
    ...
    for (const pattern of continuedWorkPatterns) {
        ...
        if (matches) {
            count += matches.length;
            examples.push(...matches.slice(0, 2));
            if (!types.includes('continued_work'))
                types.push('continued_work');
        }
    }
    // Research connection
...
function detectTrajectory(text) {
    const examples = [];
    let explicitCount = 0;
    let impliedCount = 0;
...
function detectContinuedCommitment(text) {
    const patterns = [
...
function detectUCMentions(text) {
    const patterns = [
        // Formal UC mentions
...
function detectCredibleNextSteps(text) {
    const patterns = [
...
function detectGenericMission(text) {
    const patterns = [
...
//# sourceMappingURL=fitTrajectoryAnalyzer.js.map