 ...
 */
export function analyzeSpecificity(essayText) {
    // Numbers and metrics
    ...
    if (timeDuration)
        score += 0.5;
    if (moneyAmounts || peopleCounts)
        score += 0.5;
    // Proper nouns (+2 max)
    if (properNouns.all.length >= 5)
        score += 1.0;
    else if (properNouns.all.length >= 3)
        score += 0.6;
    else if (properNouns.all.length >= 1)
        score += 0.3;
    if (properNouns.people.length > 0)
        score += 0.5;
    if (properNouns.places.length > 0)
        score += 0.3;
    if (properNouns.organizations.length > 0)
        score += 0.2;
    ...
    if (beforeAfter.count >= 2)
        score += 1.0;
    else if (beforeAfter.count >= 1)
        score += 0.5;
    // Penalties for vagueness (-2 max)
    if (vaguePhrases.length >= 5)
        score -= 1.5;
    else if (vaguePhrases.length >= 3)
        score -= 1.0;
    else if (vaguePhrases.length >= 1)
        score -= 0.5;
    // Cap at 0-10
...
function detectBeforeAfter(text) {
    const patterns = [
        /before.*?after/gi,
        /used to.*?now/gi,
...
//# sourceMappingURL=specificityAnalyzer.js.map