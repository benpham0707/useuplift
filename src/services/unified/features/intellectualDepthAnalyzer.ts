...
export interface IntellectualDepthAnalysis {
  ...
  // Berkeley alignment
  berkeley_fit: number; // 0-10 (r=0.94 correlation!)
  berkeley_signals: string[]; // Detected Berkeley-specific patterns

  // Components detected
  academic_fields_mentioned: string[];           // "psychology", "economics", "neuroscience"
  theories_concepts_referenced: string[];        // "Tuckman's model", "game theory", "PID control"
  research_activities: string[];                 // "researched", "studied", "investigated"
  questions_asked: string[];                     // Curiosity signals
  interdisciplinary_connections: string[];       // Cross-domain thinking
  self_directed_learning: string[];              // "taught myself", "explored"
  innovations_created: string[];                 // Novel contributions
  measurements_taken: string[];                  // Data-driven approach
  dissemination_activities: string[];            // Presented, published, shared
  ongoing_curiosity: string[];                   // Unresolved questions
  ...
  // Research alignment
  stanford_intellectual_vitality: boolean; // Asks questions, pursues learning
  mit_maker_match: boolean;               // Makes/fixes things, hands-on
  harvard_genuine_curiosity: boolean;     // Beyond grades
...
export function analyzeIntellectualDepth(text: string): IntellectualDepthAnalysis {
  ...
  const level = calculateIntellectualLevel(
    hasAcademicField,
    hasTheoryReference,
    hasResearch,
    hasInnovation,
  ...
  // Calculate score
  const score = calculateIntellectualScore(
    level,
    academicFields.length,
    theoriesReferenced.length,
    researchActivities.length,
    innovationsCreated.length,
    hasMeasurement,
    hasDissemination
  );
  ...
  const intellectualQuotes = extractIntellectualQuotes(
    text,
    academicFields,
    theoriesReferenced,
    researchActivities,
  ...
  return {
    ...
    weaknesses,
    current_level: level,
    next_level: Math.min(5, level + 1) as IntellectualLevel,
...
function detectAcademicFields(text: string): string[] {
  const found: string[] = [];
...
function detectTheoriesAndConcepts(text: string): string[] {
  const found: string[] = [];
  const lowerText = text.toLowerCase();
...
function detectResearchActivities(text: string): string[] {
  const researchVerbs = [
...
function detectQuestionsAsked(text: string): string[] {
...
function detectInterdisciplinaryConnections(text: string): string[] {
...
function detectSelfDirectedLearning(text: string): string[] {
...
function detectInnovationsCreated(text: string): string[] {
...
function detectMeasurementsTaken(text: string): string[] {
...
function detectDisseminationActivities(text: string): string[] {
...
function detectOngoingCuriosity(text: string): string[] {
...
function calculateIntellectualLevel(
  hasAcademicField: boolean,
  hasTheoryReference: boolean,
  hasResearch: boolean,
  hasInnovation: boolean,
  hasMeasurement: boolean,
  hasDissemination: boolean,
...
function calculateIntellectualScore(
  ...
  if (fieldCount >= 2) score += 0.5;
  if (theoryCount >= 2) score += 0.5;
  if (researchCount >= 2) score += 0.5;
  if (innovationCount >= 2) score += 0.5;
  if (hasMeasurement) score += 0.3;
  if (hasDissemination) score += 0.5;

  return Math.min(10, score);
}

function getTier(level: IntellectualLevel): 'scholarly' | 'innovative' | 'curious' | 'competent' | 'basic' {
  const tiers = {
    5: 'scholarly' as const,
    4: 'innovative' as const,
    3: 'curious' as const,
    2: 'competent' as const,
    1: 'basic' as const
  };
  return tiers[level];
}
...
function calculateBerkeleyFit(
  ...
  if (hasTheoryReference) fit += 0.5; // Academic rigor
  if (hasResearch) fit += 0.5;        // Research potential
  if (hasInnovation) fit += 0.5;      // Innovation mindset

  return Math.min(10, fit);
}

function identifyBerkeleySignals(
  text: string,
  academicFields: string[],
  theoriesReferenced: string[]
): string[] {
...
function extractIntellectualQuotes(
  text: string,
  academicFields: string[],
  theoriesReferenced: string[],
  researchActivities: string[],
  innovationsCreated: string[]
): string[] {
  const quotes: string[] = [];
...
function identifyIntellectualStrengths(
  level: IntellectualLevel,
  academicFields: string[],
  theoriesReferenced: string[],
  researchActivities: string[],
...
function identifyIntellectualWeaknesses(
  level: IntellectualLevel,
  hasAcademicField: boolean,
  hasTheoryReference: boolean,
  hasResearch: boolean,
...
function generateUpgradeToNextLevel(
  currentLevel: IntellectualLevel,
  hasAcademicField: boolean,
  hasTheoryReference: boolean,
  hasResearch: boolean,
  hasInnovation: boolean,
  hasDissemination: boolean
): IntellectualDepthAnalysis['upgrade_to_next'] {

  const nextLevel = Math.min(5, currentLevel + 1) as IntellectualLevel;
  ...
  if (nextLevel === 3) {
    if (!hasAcademicField) whatToAdd.push('Connect to academic field');
    if (!hasResearch) whatToAdd.push('Show research/exploration');
  ...
  if (nextLevel === 4) {
    if (!hasTheoryReference) whatToAdd.push('Reference academic theory/concept');
    if (!hasInnovation) whatToAdd.push('Create framework/approach based on theory');
  ...
  if (nextLevel === 5) {
    if (!hasDissemination) whatToAdd.push('Present/publish/share findings');
...
function generateWorldClassUpgrade(): IntellectualDepthAnalysis['upgrade_to_world_class'] {
  return {
...
function getAOAssessment(
  level: IntellectualLevel,
  berkeleyFit: number,
  stanfordIntellectualVitality: boolean
): string {
...
// Already exported inline above (line 222)
