...
export interface VividnessAnalysis {
  // Overall assessment
  overall_vividness_score: number; // 0-10
  ...
  senses: {
    ...
    total_senses_engaged: number; // 0-5
  };
  ...
  action_specificity: {
    ...
    strong_verb_ratio: number; // specific / (specific + generic)
    score: number; // 0-10
  };
  ...
  scenes: {
    ...
    scene_count: number;
    best_scene_quality: number; // 0-10
  };

  // Evidence
  best_vivid_moments: string[];
  weakest_moments: string[]; // Where it tells instead of shows
  ...
  // Research alignment
  harvard_pattern_match: boolean; // Concrete scene present
...
export function analyzeVividness(text: string): VividnessAnalysis {
  ...
  const overallScore = calculateOverallVividness(
    totalSenses,
    temporalAnalysis.score,
    spatialAnalysis.score,
    dialogueAnalysis.score,
    innerMonologueAnalysis.score,
    actionAnalysis.score,
    sceneAnalysis.best_scene_quality
  );
  ...
  const upgradeTo10 = generateUpgradePath(
    totalSenses,
    temporalAnalysis,
    spatialAnalysis,
    dialogueAnalysis,
...
function analyzeSense(text: string, senseType: keyof typeof SENSORY_PATTERNS): SenseAnalysis {
  ...
  const quality: 'vivid' | 'adequate' | 'weak' = count >= 3 ? 'vivid' : count >= 1 ? 'adequate' : 'weak';
...
function analyzeTemporalSpecificity(text: string): VividnessAnalysis['temporal'] {
  const examples: string[] = [];
  ...
  let precisionLevel: 'exact' | 'specific' | 'vague' | 'none' = 'none';
  let score = 3; // Default
...
function analyzeSpatialSpecificity(text: string): VividnessAnalysis['spatial'] {
  const examples: string[] = [];
  ...
  let precisionLevel: 'precise' | 'specific' | 'generic' | 'none' = 'none';
  let score = 3; // Default
...
function analyzeDialogue(text: string): VividnessAnalysis['dialogue'] {
  ...
  let quality: 'reveals_character' | 'conversational' | 'info_dump' | 'formal' | 'none' = 'none';
  let score = 0;
  ...
  if (hasDialogue) {
    ...
    const revealsCharacter = matches.some(m =>
      characterRevealingPhrases.some(p => p.test(m))
    );
...
function analyzeInnerMonologue(text: string): VividnessAnalysis['inner_monologue'] {
...
function analyzeActionSpecificity(text: string): VividnessAnalysis['action_specificity'] {
  const specificVerbs: string[] = [];
  const genericVerbs: string[] = [];
...
function analyzeScenes(
  text: string,
  visualAnalysis: SenseAnalysis,
  temporalAnalysis: VividnessAnalysis['temporal'],
  spatialAnalysis: VividnessAnalysis['spatial']
): VividnessAnalysis['scenes'] {

  const hasConcreteScene = (
    visualAnalysis.present &&
...
function calculateOverallVividness(
  totalSenses: number,
  temporalScore: number,
  spatialScore: number,
  dialogueScore: number,
  innerMonologueScore: number,
  actionScore: number,
  bestSceneQuality: number
): number {
  ...
  const overallScore = (
    senseScore * 0.30 +           // 30% weight on senses
...
function getVividnessTier(score: number): 'world_class' | 'strong' | 'adequate' | 'weak' | 'resume_style' {
  if (score >= 9) return 'world_class';
...
function extractBestVividMoments(
  text: string,
  visual: SenseAnalysis,
  auditory: SenseAnalysis,
  tactile: SenseAnalysis
...
function extractWeakestMoments(text: string): string[] {
  const weak: string[] = [];
...
function identifyVividnessStrengths(
  totalSenses: number,
  temporal: VividnessAnalysis['temporal'],
  dialogue: VividnessAnalysis['dialogue'],
  action: VividnessAnalysis['action_specificity']
): string[] {
...
function identifyVividnessWeaknesses(
  totalSenses: number,
  temporal: VividnessAnalysis['temporal'],
  spatial: VividnessAnalysis['spatial'],
  dialogue: VividnessAnalysis['dialogue']
): string[] {
...
function generateUpgradePath(
  ...
  spatial: VividnessAnalysis['spatial'],
  dialogue: VividnessAnalysis['dialogue'],
  innerMonologue: VividnessAnalysis['inner_monologue']
): VividnessAnalysis['upgrade_to_10'] {

  const missingElements: string[] = [];

  if (totalSenses < 3) missingElements.push(`Add ${3 - totalSenses} more senses (visual, auditory, tactile)`);
  if (temporal.precision_level !== 'exact') missingElements.push('Add exact time ("11:47 PM", "Tuesday afternoon")');
  if (spatial.precision_level !== 'precise') missingElements.push('Add precise location ("empty robotics lab" not "the room")');
  if (!dialogue.has_dialogue) missingElements.push('Add conversational dialogue');
  if (!innerMonologue.has_inner_monologue) missingElements.push('Add inner monologue (show your thinking)');
...
// Already exported inline above (line 231)
