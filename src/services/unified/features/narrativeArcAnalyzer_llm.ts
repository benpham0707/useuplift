...
export interface NarrativeArcAnalysis {
  ...
  reasoning: {
    structure_analysis: string; // Is it chronological, in media res, etc?
    tension_analysis: string; // Where does the tension come from?
    stakes_analysis: string; // What is at risk?
    pacing_analysis: string; // Scene vs summary balance
  };
  ...
  // Evidence
  evidence_quotes: string[]; // Key moments (inciting incident, climax)
  evaluator_note: string;

  // Detailed analysis
  arc_components: {
    inciting_incident: { present: boolean; description: string };
    rising_action: { present: boolean; description: string };
    climax: { present: boolean; description: string };
    falling_action: { present: boolean; description: string };
    resolution: { present: boolean; description: string };
  };

  pacing_check: {
    scene_count: number; // Estimate of distinct scenes
...
export async function analyzeNarrativeArc(essayText: string): Promise<NarrativeArcAnalysis> {
