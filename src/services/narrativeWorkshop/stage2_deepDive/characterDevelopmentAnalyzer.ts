...
function buildCharacterDevelopmentPrompt(
  essayText: string,
  essayType: string
): string {
 ...
 */
export async function analyzeCharacterDevelopment(
  ...
  try {
    ...
    let analysis: CharacterDevelopmentAnalysis;

    if (typeof response.content === 'string') {
      const jsonMatch = response.content.match(/\{[\s\S]*\}/);
      if (!jsonMatch) throw new Error('No JSON found in character development response');
 ...
 */
export function analyzeCharacterDeterministic(essayText: string): {
  ...
  const interiorityMarkers: string[] = [];
 ...
 */
export function detectInteriority(essayText: string): {
  present: boolean;
  markers: string[];
  ...
  let depth: 'none' | 'minimal' | 'moderate' | 'deep';

  if (count === 0) depth = 'none';
  else if (count < 2) depth = 'minimal';
  else if (count < 5) depth = 'moderate';
  else depth = 'deep';
 ...
 */
export function detectEmotionStyle(essayText: string): {
  ...
  let dominantStyle: 'physical' | 'behavioral' | 'abstract' | 'mixed' | 'minimal';
 ...
 */
export function detectDialogue(essayText: string): {
  present: boolean;
 ...
 */
export function assessProtagonistClarity(essayText: string): {
  identityMarkersFound: string[];
  establishedEarly: boolean; // within first 100 words
  ...
  let clarity: 'clear' | 'moderate' | 'vague';
  if (markers.length >= 2 && establishedEarly) clarity = 'clear';
  else if (markers.length >= 1) clarity = 'moderate';
  else clarity = 'vague';
