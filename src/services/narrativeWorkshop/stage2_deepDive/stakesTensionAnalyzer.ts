...
function buildStakesTensionPrompt(
  essayText: string,
  essayType: string
): string {
 ...
 */
export async function analyzeStakesTension(
  ...
  try {
    ...
    let analysis: StakesTensionAnalysis;

    if (typeof response.content === 'string') {
      const jsonMatch = response.content.match(/\{[\s\S]*\}/);
      if (!jsonMatch) throw new Error('No JSON found in stakes/tension response');
 ...
 */
export function analyzeStakesTensionDeterministic(essayText: string): {
 ...
 */
export function detectStakesEstablishment(essayText: string): {
  stakesPresent: boolean;
 ...
 */
export function assessConflictType(essayText: string): {
  hasInternal: boolean;
  hasExternal: boolean;
  conflictType: 'internal' | 'external' | 'both' | 'none';
  ...
  let conflictType: 'internal' | 'external' | 'both' | 'none';
  if (hasInternal && hasExternal) conflictType = 'both';
  else if (hasInternal) conflictType = 'internal';
  else if (hasExternal) conflictType = 'external';
  else conflictType = 'none';

  // Complexity heuristic
  let complexity: 'simple' | 'moderate' | 'complex';
  if (hasInternal && hasExternal) complexity = 'complex';
  else if (hasInternal || hasExternal) complexity = 'moderate';
 ...
 */
export function detectTensionTechniques(essayText: string): {
  techniques: string[];
  ...
  let tensionLevel: 'none' | 'low' | 'moderate' | 'high';
  if (techniques.length === 0) tensionLevel = 'none';
  else if (techniques.length === 1) tensionLevel = 'low';
  else if (techniques.length <= 3) tensionLevel = 'moderate';
  else tensionLevel = 'high';
