...
// PROMPT BUILDER
// ============================================================================
function buildClimaxTurningPointPrompt(essayText, essayType) {
 ...
 */
export async function analyzeClimaxTurningPoint(input, essayType) {
    ...
    try {
        ...
        if (typeof response.content === 'string') {
            ...
            if (!jsonMatch)
                throw new Error('No JSON found in climax analysis response');
 ...
 */
export function detectTurningPointsDeterministic(essayText) {
 ...
 */
export function extractMiddleSection(essayText) {
    const words = essayText.split(/\s+/);
    const totalWords = words.length;
    const skipFirst = Math.floor(totalWords * 0.2);
    const skipLast = Math.floor(totalWords * 0.2);
 ...
 */
export function findTurningPointSentence(essayText) {
...
//# sourceMappingURL=climaxTurningPointAnalyzer.js.map