...
// PROMPT BUILDER
// ============================================================================
function buildConclusionReflectionPrompt(essayText, essayType) {
 ...
 */
export async function analyzeConclusionReflection(input, essayType) {
    ...
    try {
        ...
        if (typeof response.content === 'string') {
            ...
            if (!jsonMatch)
                throw new Error('No JSON found in conclusion analysis response');
 ...
 */
export function analyzeConclusionDeterministic(essayText) {
 ...
 */
export function extractConclusion(essayText) {
    const words = essayText.split(/\s+/);
    const conclusionStart = Math.floor(words.length * 0.80);
 ...
 */
export function detectMicroToMacro(essayText) {
    const text = essayText.toLowerCase();
 ...
 * Extract last paragraph
 */
export function getLastParagraph(essayText) {
    const paragraphs = essayText.split('\n\n').filter(p => p.trim().length > 0);
    return paragraphs[paragraphs.length - 1] || '';
}
//# sourceMappingURL=conclusionReflectionAnalyzer.js.map