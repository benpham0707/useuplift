              ...
              <div className="mb-4 pb-4 border-b">
                ...
                <div className="mt-4 space-y-2">
                  {goodDimensions.length > 0 && (
                    <div>
                      <div className="space-y-1">
                        {(showAllStrong ? goodDimensions : goodDimensions.slice(0, 2)).map(dim => (
                          <button 
                            key={dim.id}
                            onClick={() => scrollToDimension(dim.id)}
                            className="w-full flex items-center justify-between px-3 py-1.5 rounded-md hover:bg-emerald-50 dark:hover:bg-emerald-950/20 transition-colors group"
                          >
                            <div className="flex items-center gap-2">
                              <CheckCircle className="w-3.5 h-3.5 text-emerald-500" />
                              <span className="text-xs font-medium group-hover:underline underline-offset-2">{dim.name}</span>
                            </div>
                            <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">{dim.score}/{dim.maxScore}</span>
                          </button>
                        ))}
                      </div>
                      {goodDimensions.length > 2 && (
                        <button 
                          onClick={() => setShowAllStrong(!showAllStrong)}
                          className="text-xs text-muted-foreground hover:text-foreground ml-3 mt-1"
                        >
                          {showAllStrong ? '← Show less' : `+${goodDimensions.length - 2} more strong...`}
                        </button>
                      )}
                    </div>
                  )}
                  ...
                  {needsWorkDimensions.length > 0 && (
                    <div>
                      <div className="space-y-1">
                        {(showAllNeedsWork ? needsWorkDimensions : needsWorkDimensions.slice(0, 2)).map(dim => (
                          <button 
                            key={dim.id}
                            onClick={() => scrollToDimension(dim.id)}
                            className="w-full flex items-center justify-between px-3 py-1.5 rounded-md hover:bg-amber-50 dark:hover:bg-amber-950/20 transition-colors group"
                          >
                            <div className="flex items-center gap-2">
                              <AlertTriangle className="w-3.5 h-3.5 text-amber-500" />
                              <span className="text-xs font-medium group-hover:underline underline-offset-2">{dim.name}</span>
                            </div>
                            <span className="text-xs font-semibold text-amber-600 dark:text-amber-400">{dim.score}/{dim.maxScore}</span>
                          </button>
                        ))}
                      </div>
                      {needsWorkDimensions.length > 2 && (
                        <button 
                          onClick={() => setShowAllNeedsWork(!showAllNeedsWork)}
                          className="text-xs text-muted-foreground hover:text-foreground ml-3 mt-1"
                        >
                          {showAllNeedsWork ? '← Show less' : `+${needsWorkDimensions.length - 2} more...`}
                        </button>
                      )}
                    </div>
                  )}
                    ...
                    <div className="space-y-1">
                      {criticalDimensions.map(dim => (
                        <button 
                          key={dim.id}
                          onClick={() => scrollToDimension(dim.id)}
                          className="w-full flex items-center justify-between px-3 py-1.5 rounded-md hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors group"
                        >
                          <div className="flex items-center gap-2">
                            <XCircle className="w-3.5 h-3.5 text-red-500" />
                            <span className="text-xs font-medium group-hover:underline underline-offset-2">{dim.name}</span>
                          </div>
                          <span className="text-xs font-semibold text-red-600 dark:text-red-400">{dim.score}/{dim.maxScore}</span>
                        </button>
                      ))}
                    </div>
                  )}
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <TooltipProvider delayDuration={750}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        onClick={() => { if (criticalDimensions.length > 0) scrollToDimension(criticalDimensions[0].id); }}
                    ...
                    {criticalDimensions.length > 0 && (
                      <TooltipContent side="bottom" className="max-w-xs">
                        <div className="space-y-1">
                          ...
                          {criticalDimensions.map((dim) => (
                            <button key={dim.id} onClick={() => scrollToDimension(dim.id)} className="w-full text-left text-xs hover:bg-muted/50 rounded p-1 transition-colors group">
                              ...
                              {dim.issues.length > 0 && (<p className="text-muted-foreground ml-3 mt-0.5">{dim.issues[0].title}</p>)}
                            </button>
                          ))}
                        </div>
                      </TooltipContent>
                ...
                <TooltipProvider delayDuration={750}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        onClick={() => { if (needsWorkDimensions.length > 0) scrollToDimension(needsWorkDimensions[0].id); }}
                ...
                <TooltipProvider delayDuration={750}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        onClick={() => { if (goodDimensions.length > 0) scrollToDimension(goodDimensions[0].id); }}
