    ...
    <div ref={scoreContainerRef} className="relative space-y-0">
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
        ...
        <div className="flex flex-col items-center">
          <div className="w-full text-center px-3 py-2.5 rounded-lg border border-white/30 bg-white/15">
            <div className={`text-3xl font-bold ${getScoreCardColor(overallScore)}`}>
          ...
          <span className="text-[9px] text-white/40 mt-0.5">
            {harvardRating ? `Scale ${harvardRating}` : ''}{overallConfidence ? ` · ${Math.round(overallConfidence * 100)}%` : ''}
          </span>
        </div>
        {scoreCards.map((card, idx) => (
          <button
            ...
            className={`w-full text-center px-3 py-2.5 rounded-lg border transition-colors duration-200 cursor-pointer ${
          ...
          >
            <div className={`text-2xl font-bold ${getScoreCardColor(card.score)}`}>
      ...
      {/* Expansion panel — auto height */}
      <div
        className="grid transition-[grid-template-rows] duration-300 ease-out"
        style={{ gridTemplateRows: expandedScoreCard !== null ? '1fr' : '0fr' }}
      >
        <div className="overflow-hidden">
          {expandedScoreCard !== null && (
            <div className="relative mt-2">
              {caretLeftPx !== null && (
                <div
                  className="absolute -top-1.5 w-3 h-3 bg-white/15 border-t border-l border-white/30 rotate-45 z-10"
                  style={{ left: caretLeftPx - 6 }}
                />
              )}
              <div className="rounded-lg border border-white/25 bg-white/15 px-4 py-3">
                <h4 className="font-semibold text-white text-sm mb-1.5">{scoreCards[expandedScoreCard].label}</h4>
                <p className="text-xs text-white/85 leading-relaxed mb-2">{scoreCards[expandedScoreCard].rationale}</p>
                <div className="bg-white/10 rounded-md px-3 py-2">
                  <p className="text-[10px] font-semibold text-white/60 uppercase tracking-wider mb-1">How to improve</p>
                  <ul className="space-y-1">
                    {scoreCards[expandedScoreCard].improvements.map((imp, i) => (
                      <li key={i} className="flex items-start gap-1.5 text-xs text-white/85">
                        <TrendingUp className="h-3 w-3 text-teal-400 mt-0.5 flex-shrink-0" />
                        <span>{imp}</span>
                      </li>
                    ))}
                  </ul>
